...
export default function Judging() {
  ...
  const fetchContestsByTeams = useMapContestToTeamStore(
    (state) => state.fetchContestsByTeams
  );
  const clearContests = useMapContestToTeamStore(
    (state) => state.clearContests
  );

  /**
   * Hide preloader once data has actually loaded.
   * This way: when preloader fades, the dashboard is ready → no 1s blank gap.
   */
  useEffect(() => {
    if (hasLoaded) {
  ...
  }, [hasLoaded, setShowPreloader, setPreloaderProgress]);
  ...
  const fetchAllClustersForJudge = useCallback(
    async (judgeId: number, forceRefresh: boolean = false) => {
      try {
        ...
        if (allClusters && allClusters.length > 0) {
          ...
          // Build base currentCluster
          const currentClusterToSet:
            | (ClusterWithContest & {
                judgeHasChampionshipByContest?: { [key: number]: boolean };
                hasAnyTeamAdvancedByContest?: { [key: number]: boolean };
              })
            | null = clustersToShow[0]
            ? { ...clustersToShow[0] }
            : null;
          ...
          /**
           * Fetch all teams for each contest to check if any team has advanced.
          ...
          // Merge advancement info into currentCluster
          if (currentClusterToSet) {
          ...
          // Tabulation check remains unchanged
    ...
    [fetchAllClustersByJudgeId, fetchTeamsByClusterId]
  );
  ...
  useEffect(() => {
    ...
    // 1) Try to hydrate from cached teams in the store first (no skeletons)
    ...
    // 2) Decide if we need a background refresh
    const judgeIdChanged = lastLoadedJudgeIdRef.current !== judgeIdNumber;
  
    if (judgeIdChanged || !hasCachedTeams) {
      // New judge OR no cache → fetch fresh
      lastLoadedJudgeIdRef.current = judgeIdNumber;
  
      // ❗ Don't clear teams here, so UI doesn't flicker when coming back
      // setTeams([]);
      // setHasLoaded(false);
      // isInitialLoadRef.current = true;  // not needed for soft refresh
  ...
  useEffect(() => {
    const handleDataChange = async (event: DataChangeEvent) => {
      ...
      const resetAndFetch = async (force: boolean = false) => {
        setTeams([]);
        setHasLoaded(false);
        isInitialLoadRef.current = true;
        await fetchAllClustersForJudge(judgeIdNumber, force);
      };
  ...
  }, [judgeIdNumber, currentCluster, fetchAllClustersForJudge]);
      ...
      <Box sx={{ pb: 8, backgroundColor: "#fafafa", minHeight: "100vh" }}>
        ...
        <Container maxWidth="lg" sx={{ px: { xs: 2, sm: 3 } }}>
          {mapClusterToTeamError ? (
            ...
            <Box /* 👈 always visible now, no opacity gating */
            >
