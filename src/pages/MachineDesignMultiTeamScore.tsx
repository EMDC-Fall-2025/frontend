    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const [clusterResults, teamResults] = await Promise.all([
          // Fetch scoresheets for all clusters (sheetType 3 = Machine Design)
          Promise.all(clusterIds.map(async (clusterId) => {
            try {
              ...
              // Filter by sheetType 3 (Machine Design) and transform
              return scoreSheets
                .filter((item: any) => item.mapping?.sheetType === 3)
                .map((item: any) => ({
                  ...item.scoresheet,
