...
const JudgeDashboardTable = React.memo(function JudgeDashboardTable(
  props: IJudgeDashboardProps
) {
  ...
  const { mappings, fetchScoreSheetsByJudge, isLoadingMapScoreSheet } =
  ...
  const isPreliminarySheet = (sheetType: number) =>
    sheetType >= 1 && sheetType <= 5;
  ...
  const isPreliminaryScoresheet = (_team: Team, sheetType: number) => {
    ...
    const anyTeamInContestAdvanced = teamContest
      ? hasAnyTeamAdvancedByContest.get(teamContest.id) || false
      : false;

    return (
      isJudge && isPreliminarySheet(sheetType) && anyTeamInContestAdvanced
    );
  };
  ...
  const visibleTeams: Team[] = useMemo(() => {
    ...
    return teams.filter((team) => {
      ...
      if (
        contestForTeam.is_open === false &&
        contestForTeam.is_tabulated === true
      ) {
        return false;
      }
  ...
  const getIsSubmitted = useCallback(
    (judgeId: number, teamId: number, sheetType: number) => {
  ...
  const checkAllPreliminaryScoresheetsCompleted = (teamId: number) => {
    ...
    for (const sheetType of preliminaryTypes) {
      if (hasScoresheet(judge.id, teamId, sheetType)) {
        const scoresheet = multipleScoreSheets?.find(
          (sheet) =>
            sheet.teamId === teamId &&
  ...
  /**
   * Precompute: are we in a championship/redesign cluster?
   */
  const isChampionshipOrRedesignCluster = useMemo(() => {
    if (!currentCluster) return false;
    const name = currentCluster.cluster_name?.toLowerCase() || "";
    return (
  ...
  /**
   * Precompute: which teams have advanced to championship?
   */
   ...
   */
  const hasOnlyPreliminaryScoresheets = useCallback(
    (teamId: number) => {
      if (!isChampionshipOrRedesignCluster) {
        return false;
      }

      const teamInChampionshipCluster = advancedTeamIds.has(teamId);

      if (teamInChampionshipCluster) {
        const hasChampionshipScoresheets =
          judge &&
          (hasScoresheet(judge.id, teamId, 6) ||
            hasScoresheet(judge.id, teamId, 7));

        if (hasChampionshipScoresheets) {
          const hasIncompleteChampionshipScoresheets =
            judge &&
            ((hasScoresheet(judge.id, teamId, 6) &&
              !getIsSubmitted(judge.id, teamId, 6)) ||
              (hasScoresheet(judge.id, teamId, 7) &&
                !getIsSubmitted(judge.id, teamId, 7)));

          return !hasIncompleteChampionshipScoresheets;
        } else {
          const allPreliminaryCompleted =
            checkAllPreliminaryScoresheetsCompleted(teamId);
    ...
    [
      isChampionshipOrRedesignCluster,
      advancedTeamIds,
      judge,
      hasScoresheet,
      getIsSubmitted,
      checkAllPreliminaryScoresheetsCompleted,
    ]
  );
   ...
   */
  const hasMappingsForJudge = useMemo(() => {
    ...
    if (judge?.id) {
      try {
        const response = await api.get(
          `/api/mapping/contestToJudge/judge-contests/${judge.id}/`
        );
      ...
      <>
        {!isSubmitted ? (
          <Button
            variant="contained"
            onClick={() =>
              isEditable ? navigate(`/${url}/${judge.id}/${team.id}/`) : null
            }
            disabled={!isEditable}
            sx={{
              mb: { xs: 0.5, sm: 1 },
          ...
          >
            ...
            {getTotal(judge?.id, team.id, type)}
          </Button>
        ) : (
          <Button
            variant="contained"
    ...
    <>
          ...
          >
            <TableBody>
              {visibleTeams.map((team: Team) => {
                const isPreliminaryTeam = hasOnlyPreliminaryScoresheets(
                  team.id
                );
                return (
      ...
      >
          ...
          {allContests.length > 0 ? (
            ...
            >
              {allContests.map((contest) => {
                ...
                const anyTeamAdvanced =
                  hasAnyTeamAdvancedByContest.get(contest.id) || false;
        ...
        <DialogActions
          sx={{
            ...
            background:
              "linear-gradient(135deg, rgba(76, 175, 80, 0.01) 0%, rgba(76, 175, 80, 0.02) 100%)",
            justifyContent: "center",
          }}
        >
          <Button
      ...
      >
        ...
        >
          <FormControl
            component="fieldset"
            sx={{ mt: { xs: 0.5, sm: 1 }, width: "100%", maxWidth: "400px" }}
          >
            <FormLabel
              component="legend"
              sx={{
