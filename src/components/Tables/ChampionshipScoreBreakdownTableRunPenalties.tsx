...
/**
 * Championship Score Breakdown — Run Penalties table
 * - Reads penalty breakdown from championship scoresheets (fields 26-42)
 * - Renders a read-only table of penalties, types, point values, and deducted points
 */
export default function ChampionshipScoreBreakdownTableRunPenalties() {
  ...
  return scoreSheetBreakdown ? (
    ...
    >
      ...
      >
        ...
        <TableBody>
          {runPenaltiesQuestionsList.map((penalty, index) => {
            // Map preliminary penalty field to championship penalty field
            // field1 -> field26, field2 -> field27, etc. (skipping empty id 9)
            const championshipField = `field${26 + index}`;
            return (
              <PenaltiesRow
                key={penalty.field}
